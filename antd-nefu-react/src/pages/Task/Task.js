...
class UpdateForm extends PureComponent {
  ...
  handleNext = currentStep => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      if (err) return;
      const formVals = { id:oldValue.id, ...fieldsValue };
      console.log("formVals",formVals)
      axios.post("http://192.168.43.245:8080/api/manager/reply",(formVals)).then(res=>{
        console.log("res",res)
  ...
  children(){
    ...
    axios.get(`http://192.168.43.245:8080/api/replay/${id}`).then(res=>{
      console.log("res",res)
    ...
    return   (<p style={{color:"Right"}}>{this.state.answer}</p>)
  }
  cancelRenderFooter = currentChange => {
    ...
    return [
      ...
      <Button key="submit" type="primary" onClick={() => handleCancelModalVisible(false, values)}>
        确定
    </Button>,
    ];
  };d
  render() {
    ...
    return (
      ...
      >
       ...
       <FormItem key="reply" {...this.formLayout} label="回复任务">
...
class TableList extends PureComponent {
        ...
        if(record.status===0){
          return(
            <Fragment>
              ...
              <span style={{width:90}}>查看回复</span>
            </Fragment>
            )
        }else{
          return(
            <Fragment>
              <span style={{width:90}}>回复</span>
              <Divider type="vertical" />
              <a onClick={() => this.handleCancelModalVisible(true, record)}>查看回复</a>
            </Fragment>
            )
        }
      },
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'taskrule/fetchCurrent',
    });
 
    dispatch({
      type: 'taskrule/taskfetch',
       payload: params,
    });
